...
8-10 c: zchrckkcqcr
2-6 h: thvbvh'''
passwords_and_policies = passwords_and_policies.split()
# All terms seperation in order of number, letter, password, in that order

n=3
password_with_policy = [passwords_and_policies[k:k+n] for k in range (0, 
                        len(passwords_and_policies), n)] 
# We are now coupling the terms in the list together
valid_count = 0
for group in password_with_policy:
    amount, letter, password = group[0], group[1][0], group[2]
    x = amount.index('-')
    lower_range = int(amount[:x])
    upper_range = int(amount[x+1:])
# Separates each element type into 3 groups 
    count = 0
    for character in password:
        if character == letter:
            count += 1
    if lower_range <= count and count <= upper_range:
        valid_count += 1
    print(valid_count)
# Use this for part 2
#   count = 0
#   first_letter = password[lower_range - 1] == letter
#   last_letter = password[upper_range - 1] == letter
#   if first_letter and not last_letter or last_letter and not first_letter:
#       valid_count +=1
#   print(valid_count)
